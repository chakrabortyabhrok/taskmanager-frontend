...
export default function AIChatWidget({ token }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 font-sans">
      ...
      {isOpen && (
        <div className="w-80 sm:w-96 h-96 flex flex-col bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-200">
          {/* Header */}
            ...
            {/* Loading State */}
            {
              loading && (
                <div className="flex justify-start">
                  <div className="max-w-[80%] px-3.5 py-2 rounded-xl text-sm bg-slate-800 text-slate-300 border border-slate-700/50 rounded-bl-none">
                    Thinking...
                  </div>
                </div>
              )
